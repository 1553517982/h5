            ...
            if os.path.abspath(filePath) == filePath:
                ...
                if pre_md5conf.has_key(relativePath):
                    ...
                    if not (fileMd5 == preMd5):
                        ...
                        preVer = pre_versionconf[relativePath]
                        if not modifyList.has_key(relativePath):
                            modifyList.setdefault(relativePath, relativePath)
...
def action(filePath):
    ...
    if a <> -1:
        ...
        cmd = "pngquant.exe --force --output " + outFileName + " --quality " + pngQualityValue + "-" + pngQualityValue + " %s" % (
            filePath)
        os.popen(cmd)
...
def copyScripts():
    ...
    if not os.path.exists(jsOutPutDir):
        os.makedirs(jsOutPutDir)
    if not os.path.exists(jsInputDir):
        print("project publish fail !")
        return -1;
...
def make_resJson():
    ...
    for resPath in file_path:
        ...
        # 更新旧文件
        outTmpDir = os.path.dirname(outputDir + oldName)
        if not os.path.exists(outTmpDir):
            os.makedirs(outTmpDir)
        if os.path.exists(outputDir + oldName):
            os.remove(outputDir + oldName)
        shutil.copy(inputDir + oldName, outputDir + oldName)
        ...
        # 图集 帧动画需要特殊处理
        isSheet = False
        isEffect = (resPath.find('assets\\effect') >= 0)
        isModel = (resPath.find('assets\\model') >= 0)
        # 模型或者特效json变动的时候会拷贝对应的png  png不需要单独处理
        if (isEffect or isModel):
            if not (kind == "json"):
                continue
        if (kind == "json"):
        ...
        if not os.path.exists(outputDir + newName):
            shutil.copy(inputDir + oldName, outputDir + newName)

        # 处理字体文件
        if "fnt" == kind:
            oldPng = oldName.replace(".fnt", ".png")
            newPng = newName.replace(".fnt", ".png")
            if os.path.exists(outputDir + newPng):
                os.remove(outputDir + newPng)
            shutil.copy(inputDir + oldPng, outputDir + newPng)

        isConfigData = resPath.find('assets\\data')

        # 处理配置文件 需要压缩
        if "json" == kind and isConfigData >= 0:
            oldPng = oldName.replace(".json", ".bin")
            newPng = newName.replace(".json", ".bin")

            if modifyList.has_key(resPath):
                pathfile = os.path.join(outputDir, resPath)
                nameKey = fileName + '_json'
            ...
            if not (newPng == oldPng):
                if os.path.exists(outputDir + newPng):
                    os.remove(outputDir + newPng)
                shutil.copy(outputDir + oldPng, outputDir + newPng)

            cfg2 = {}
            cfg2.setdefault("url", newPng)
            cfg2.setdefault("type", 'bin')
            cfg2.setdefault("name", fileName + "_bin")
            jsonObj["resources"].append(cfg2)

            binKeysPre = binKeys + fileName + "_bin"
            binKeys = binKeysPre + ","

        # 处理模型和图集对应的png
        if isSheet or isEffect or isModel:
            oldPng = oldName.replace(".json", ".png")
            newPng = newName.replace(".json", ".png")
            if os.path.exists(outputDir + newPng):
                os.remove(outputDir + newPng)
            shutil.copy(inputDir + oldPng, outputDir + newPng)
            ...
            cfg2 = {}
            cfg2.setdefault("url", newPng)
            cfg2.setdefault("type", 'image')
            cfg2.setdefault("name", fileName + "_png")
            jsonObj["resources"].append(cfg2)
    ...
    f.close()
    shutil.copy(outputDir + "..\\manifest.json", outputDir + "..\\manifest" + mainVersion + "." + subVersion + ".json")
...
def copyAIAndEXML():
    ...
    global outputDir
    # 拷贝AI文件夹
    print('copy AI folder')
    aiPath = outputDir + "AI"
    if os.path.exists(aiPath):
        shutil.rmtree(aiPath)
    shutil.copytree(inputDir + "AI", aiPath)
    print('copy eui_skin')
    euiPath = outputDir + "eui_skins"
    if os.path.exists(euiPath):
        shutil.rmtree(euiPath)
    shutil.copytree(inputDir + "eui_skins", euiPath)

    uiPath = outputDir + "UI"
    if os.path.exists(uiPath):
        shutil.rmtree(uiPath)
    shutil.copytree(inputDir + "UI", uiPath)

    print('copy map resource')
    uiPath = outputDir + "map"
    if os.path.exists(uiPath):
        shutil.rmtree(uiPath)
    shutil.copytree(inputDir + "map", uiPath)

    uiPath = outputDir + "thumbnail"
    if os.path.exists(uiPath):
        shutil.rmtree(uiPath)
    shutil.copytree(inputDir + "thumbnail", uiPath)

    if os.path.exists(outputDir + 'default.res.json'):
        os.remove(outputDir + 'default.res.json')
    shutil.copy(inputDir + 'default.res.json', outputDir + 'default.res.json')

    if os.path.exists(outputDir + 'default.thm.json'):
        os.remove(outputDir + 'default.thm.json')
    shutil.copy(inputDir + 'default.thm.json', outputDir + 'default.thm.json')
    print('-------resource copy finished---------')
